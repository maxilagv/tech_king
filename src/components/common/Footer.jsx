...
function MagneticSocial({ href, label, className, children }) {
  const magnetic = useMagneticHover({ strength: 0.45, max: 7 });
  const props = magnetic.inert
    ? {}
    : { ref: magnetic.ref, onMouseMove: magnetic.onMouseMove, onMouseLeave: magnetic.onMouseLeave };
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className={className} {...props}>
      {children}
    </a>
  );
}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-8 mb-16">
          ...
          <div className="lg:col-span-1">
            <h3 className="tk-gradient-text-bright inline-block text-2xl font-bold font-display tracking-tight mb-3">{BRAND_NAME}</h3>
            ...
            <div className="flex items-center gap-3">
              <MagneticSocial
                href={waHref}
                label="WhatsApp"
                className="w-9 h-9 rounded-xl bg-white/8 border border-white/12 flex items-center justify-center text-white/60 hover:text-green-400 hover:border-green-400/40 hover:bg-green-400/10 transition-colors duration-300"
              >
              ...
              <MagneticSocial
                ...
                label="Instagram"
                className="w-9 h-9 rounded-xl bg-white/8 border border-white/12 flex items-center justify-center text-white/60 hover:text-pink-400 hover:border-pink-400/40 hover:bg-pink-400/10 transition-colors duration-300"
              >
              ...
              <MagneticSocial
                ...
                label="Facebook"
                className="w-9 h-9 rounded-xl bg-white/8 border border-white/12 flex items-center justify-center text-white/60 hover:text-blue-400 hover:border-blue-400/40 hover:bg-blue-400/10 transition-colors duration-300"
              >
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-1.5">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-400/70 hover:text-blue-300 transition-colors duration-300"
              >
                weblayer.cloud
              </a>
              <a
                href="https://wa.me/541137725766"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Contactar al desarrollador por WhatsApp"
                className="ml-0.5 text-white/30 hover:text-green-400 transition-colors duration-300"
              >
